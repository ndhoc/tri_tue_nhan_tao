...
def ucs(bat_dau, dich):
    ...
    while (frontier.empty() == False):
        cost, node, duong = frontier.get()
        so_node = so_node + 1
        if (node == dich): return duong, so_node, "Tìm thấy", cost
        ds_con = tao_con(node)

        step_cost = len(ds_con)

        for child in ds_con:
            new_cost = cost + step_cost
            if (child not in reached or new_cost ):
                reached[child] = new_cost
                frontier.put((new_cost, child, duong + [child]))
    return None, so_node, "Không tìm thấy", None
