...
def local_search(bat_dau, dich):
    ...
    current = node
    duong = [current]
    c_start = dem_o_sai_khong_tinh_0(current, dich_tuple)
    ds_cost = [c_start]
    so_node = 0
    ...
    while True:
        ...
        current_cost = dem_o_sai_khong_tinh_0(current, dich_tuple)
        
        found_better = False
        for neighbor in neighbors:
            neighbor_cost = dem_o_sai_khong_tinh_0(neighbor, dich_tuple)
...
def random_restart_hill_climbing(bat_dau, dich, max_restart=50):
    ...
    for i in range(1, max_restart + 1):
        ...
        while True:
            tong_node += 1
            if current == dich_tuple:
                return duong, tong_node, f"Tìm thấy (ở lượt restart {i})", {"cost": ds_cost[-1], "values": ds_cost}
    ...
    if best_run_path:
        ...
        return duong, tong_node, f"Thất bại (sau {max_restart} lượt restart)", {"cost": ds_cost[-1], "values": ds_cost}
    return [node], tong_node, f"Thất bại (sau {max_restart} lượt restart)", {"cost": c_start, "values": [c_start]}
...
def local_beam_search(bat_dau, dich, k=5, max_steps=1000):
    ...
    for state, path in current_states:
        if state == dich_tuple:
            ds_cost = [manhattan_distance(s, dich_tuple) for s in path]
            return path, so_node, "Tìm thấy (trong tập khởi tạo)", {"cost": ds_cost[-1], "values": ds_cost}
    ...
    while step_count < max_steps:
        ...
        for state, path in current_states:
            for child in tao_con(state):
                if child not in path:
                    neighbor_states.append((child, path + [child]))
                    
        if not neighbor_states:
            current_states.sort(key=lambda x: manhattan_distance(x[0], dich_tuple))
            best_state, best_path = current_states[0]
            ds_cost = [manhattan_distance(s, dich_tuple) for s in best_path]
            return best_path, so_node, "Bế tắc (Thất bại)", {"cost": ds_cost[-1], "values": ds_cost}
        ...
        for child, path in neighbor_states:
            if child == dich_tuple:
                ds_cost = [manhattan_distance(s, dich_tuple) for s in path]
                return path, so_node, "Tìm thấy", {"cost": ds_cost[-1], "values": ds_cost}
                
        neighbor_states.sort(key=lambda x: manhattan_distance(x[0], dich_tuple))
        current_states = neighbor_states[:k]
        
    current_states.sort(key=lambda x: manhattan_distance(x[0], dich_tuple))
    best_state, best_path = current_states[0]
    ds_cost = [manhattan_distance(s, dich_tuple) for s in best_path]
    return best_path, so_node, f"Thất bại (vượt quá {max_steps} bước)", {"cost": ds_cost[-1], "values": ds_cost}
...
class PuzzleApp:
    ...
    def setup_ui(self):
        ...
        right_frame = tk.Frame(main_frame, bg="#ffffff", bd=1, relief="solid", width=300)
        right_frame.pack(side=tk.RIGHT, fill=tk.Y)
        right_frame.pack_propagate(False)
        
        control_inner = tk.Frame(right_frame, bg="#ffffff", padx=15, pady=15)
        ...
        tk.Label(control_inner, text="Tham Số Thuật Toán", font=("Segoe UI", 11, "bold"), bg="#ffffff", fg="#1e40af").pack(anchor=tk.W, pady=(8, 3))
        param_frame = tk.Frame(control_inner, bg="#ffffff")
        param_frame.pack(fill=tk.X, pady=2)
        
        tk.Label(param_frame, text="k (Beam Width):", font=("Segoe UI", 9), bg="#ffffff").grid(row=0, column=0, sticky="w", pady=2)
        self.entry_k = ttk.Entry(param_frame, width=8)
        self.entry_k.insert(0, "5")
        self.entry_k.grid(row=0, column=1, sticky="w", padx=5, pady=2)
        
        tk.Label(param_frame, text="Max Restart:", font=("Segoe UI", 9), bg="#ffffff").grid(row=1, column=0, sticky="w", pady=2)
        self.entry_restart = ttk.Entry(param_frame, width=8)
        self.entry_restart.insert(0, "50")
        self.entry_restart.grid(row=1, column=1, sticky="w", padx=5, pady=2)
    ...
    def toggle_buttons(self, state):
        ...
        self.entry_k.config(state=state)
        self.entry_restart.config(state=state)
    ...
    def _thread_thuat_toan(self, ten, ham):
        try:
            if ten == "Local Beam Search":
                try:
                    k_val = int(self.entry_k.get().strip())
                except ValueError:
                    k_val = 5
                ket_qua_raw = ham(self.bang, self.dich, k=k_val)
            elif ten == "Random Restart HC":
                try:
                    restart_val = int(self.entry_restart.get().strip())
                except ValueError:
                    restart_val = 50
                ket_qua_raw = ham(self.bang, self.dich, max_restart=restart_val)
            else:
