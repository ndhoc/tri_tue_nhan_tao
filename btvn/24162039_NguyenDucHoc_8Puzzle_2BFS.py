...
def bam_bfs1():
    ...
    dang_chay=False
    thong_bao.config(text="Đang chạy BFS phiên bản 1...")
    man_hinh.update()

    ket_qua,so_node=bfs_phien_ban_1(bang,dich)
    buoc=0

    if(ket_qua==None):
        thong_bao.config(text="BFS 1 không tìm thấy")
    else:
        thong_bao.config(text="BFS 1: số bước = "+str(len(ket_qua)-1)+" | node duyệt = "+str(so_node))
...
def bam_bfs2():
    ...
    dang_chay=False
    thong_bao.config(text="Đang chạy BFS phiên bản 2...")
    man_hinh.update()

    ket_qua,so_node=bfs_phien_ban_2(bang,dich)
    buoc=0

    if(ket_qua==None):
        thong_bao.config(text="BFS 2 không tìm thấy")
    else:
        thong_bao.config(text="BFS 2: số bước = "+str(len(ket_qua)-1)+" | node duyệt = "+str(so_node))
        hien_bang(doi_list(ket_qua[0]))

def hien_buoc():
    if(len(ket_qua)==0):
        thong_bao.config(text="Chưa chạy BFS")
        return
...
man_hinh=tk.Tk()
man_hinh.title("Mô phỏng BFS 8 Puzzle")
man_hinh.geometry("480x560")

tieu_de=tk.Label(man_hinh,text="8 Puzzle - BFS",font=("Arial",18,"bold"),fg="#222222")
tieu_de.pack(pady=6)
...
nut_bfs2=tk.Button(khung_nut,text="BFS 2",font=("Arial",10),width=13,command=bam_bfs2)
